...
### v2.4.5 面包屑定位：宿主注入声明缺失（最终根因）
cordis 对未声明服务的属性访问直接抛错（非 undefined）——防崩溃时撤掉
'settings' 声明后，软探测每次被异常打飞，命名空间自那以后从未注册。
面包屑日志 ~/.dsh/verify-reflux.loadlog 实锤。恢复声明（base bundle
保证该服务存在），软探测保留为双保险。

### v2.4.6 监听器路由修复 + 首轮语义说明
full 档监听器的 resolveRoute 此前缺会话上下文 → 打到 first-registered
死路由 → 静默降级 → 空输入下无注入。现在跟随 context.scope 的模型。
首轮注入语义：全域勾选时新会话第一轮即有（基于墓碑+通用风险综合）；
未勾选时需先跑成一次 verify（人设的开场校准）从第二轮起注入。
