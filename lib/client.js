window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Keyed occupant of `tool.call.toolview` for the three verify_* wire tools.
		 * Pure function of the turn's own data: collapsed shows a one-line verdict
		 * chip, expanded shows the distilled decision record plus the raw trace.
		 */
		...
		/**
		 * Client plugin body: claim the keyed atomic Tool views for our three wire
		 * tools. Registration is additive — any unclaimed key keeps the generic
		 * card — so this can never disturb shipped renderers.
		 */
		function apply(ctx) {
