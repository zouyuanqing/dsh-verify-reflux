window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		function VerifyConfigCard({ scope, bindError }) {
			if (!scope) {
				return h(
					"div",
					{ style: styles.card },
		...
		function apply(ctx) {
			...
			let bindError = null;
			try {
				const ss = ctx.settingsScope ?? (ctx.get ? ctx.get("settingsScope") : undefined);
				if (ss && typeof ss.bind === "function") bound = ss.bind({ namespace: NS_SETTINGS });
				else bindError = "settingsScope service missing on plugin ctx";
			} catch (err) {
				bindError = String(err);
			}
			try { window.__VR_DEBUG__ = { bound: !!bound, bindError }; } catch {}
			// 无论绑定成功与否都注册卡片：失败时卡片自身渲染诊断文案，
			// 把静默搁浅变成可见信号。
			ctx.slots.inject("settings.plugin.item", function* () {
				yield ctx.slots.register(
					{ name: "settings.plugin.item", key: NS_SETTINGS },
					function CardHost() {
						return h(VerifyConfigCard, { scope: bound, bindError: bindError });
					},
				);
			});
		}
